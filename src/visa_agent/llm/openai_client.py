...
EXTRACTION_INSTRUCTIONS = (
    ...
    "visit_purpose, uk_accommodation, estimated_trip_cost_gbp, current_address, current_address_duration, "
    "occupation_status, employer_name, employer_address, employer_phone, annual_income_gbp, funding_source, sponsor_name, sponsor_address, sponsor_relationship, "
    ...
    "employer_name, employer_address and employer_phone describe this applicant's current employer only. "
    "Use exact literal text and retain a complete current sentence identifying the employer as source_excerpt. "
    "Do not transfer a former employer's details or another person's employer into these fields; do not infer a phone country code. "
    "If known_profile._employer_question_verified names an employer field, a literal short reply can answer "
    "that actually-sent question. Use the whole current reply as its source. A need-to-check reply leaves the field "
    "missing, not guessed and not by itself an ambiguity requiring human review. "
    ...
    "and current_address when they explicitly state where they live. "
    "For current_address_duration retain the exact stated duration at the applicant's current home "
    "(for example 'about two years' or '大概两年'); do not compute a move-in date, translate the value, "
    "use time living in a country, an old address or a relative's residence. "
    "When the applicant moves, extract the explicitly supplied new home address; an omitted new-home "
    "duration is ordinary missing intake, NOT an ambiguity or reason for human review. "
    "Saying the former duration belonged to the old home is a clarification, not conflicting evidence. "
    "Do not carry the old duration forward. Incomplete dates such as "
    ...
    "sponsor_address is the literal address explicitly identified as this applicant's personal sponsor's address, "
    "not the applicant home, host address or UK accommodation. Preserve its spelling, never invent or translate "
    "address components. Include the complete current sentence identifying its owner as source_excerpt. "
    "If known_profile._sponsor_address_question_verified is true, a literal short address in the current reply "
    "can answer that sent sponsor-address question; use the whole current reply as source_excerpt. "
    "A need-to-check answer leaves sponsor_address missing, not an invented address or an ambiguity requiring a hold. "
    ...
    "Allowed topics: application = official visitor application entry/form/process; route_orientation = a "
    "request to distinguish or confirm the appropriate UK visa route, including visitor versus Student; timing = "
    "earliest application and usual decision timing, not a guarantee for their personal deadline; "
    "eligibility_overview = the general Standard Visitor eligibility framework, including a sender asking whether "
    "they appear to qualify, without deciding eligibility or predicting approval; biometrics = the Standard Visitor "
    "visa application-centre appointment, fingerprints, photograph, identity check, appointment location or documents "
    "to take; after_apply = what happens after submission, including decision notification, tracking within published "
    "processing times, or how to change, correct, cancel or withdraw a submitted application; "
    ...
    "coverage, obtaining statements and evidence of accessible funds; sponsor_support = what a "
    "personal sponsor statement should explain and how to evidence the support, available funds, "
    "relationship and, where applicable, the sponsor's lawful UK status; document_checklist = a "
    ...
    "answered by those narrowly defined topics (including approval chances, an individual legal conclusion beyond "
