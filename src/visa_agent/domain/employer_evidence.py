...
def employer_detail_is_grounded(field: str, value: str | int | bool, excerpt: str, body: str) -> bool:
    ...
    if field not in EMPLOYER_FIELDS or not isinstance(value, str) or not value.strip() or len(value) > 400:
        return False
    if field == "employer_phone" and (
            not re.fullmatch(r"\+?[\d ()\-.]+", value) or not 7 <= len(re.sub(r"\D", "", value)) <= 15):
        return False
