...
| 本地部署与开发 | 源码、依赖锁文件、启动脚本、Docker、CI、测试、虚构材料和实验报告 | 不包含 API key、OAuth token、真实客户邮件或私人案件数据库 |
...
支持的普通咨询会先获得公开指引，不需要先提交个人材料。需要处理个人资料或附件时，
申请人需要在原邮件线程收到当前处理告知并回复其中的同意参考码。
...
这个仓库提供**范围明确、可运行、可审计的本地示例和受监督 Gmail 服务**，不是公开运营版本。
自动测试全绿不等于“自然度、准确率、可靠性 100%”。仍需真实同意后的普通材料全流程、
独立用户试用、真实 WhatsApp 设备测试；复杂流水、联合账户、所有扫描件和
